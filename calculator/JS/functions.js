...
const calc = () => {
  if (result.innerText.includes(currentOperation)){
    ...
    if (result.innerText.charAt(result.innerText.length - 1) != currentOperation){
...
const insertOperation = (event) => {
  if (!result.innerText.includes("+") && !result.innerText.includes("-") && !result.innerText.includes("*") && !result.innerText.includes("/") && !result.innerText.includes("^") && !result.innerText.includes("√") && result.innerText != 0){
    result.innerText +=  event.target.name;
